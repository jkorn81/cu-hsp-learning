---
title: Researching Countries Mobility Rate during COVID-19 using Machine Learning 
  Learning
author: "HU - J.Korn"
date: "April 4, 2020"
...
setwd("C:/Users/jonat/Desktop/hu.projects/project.covid-19.google")
```
...
data <- read_excel("Modeling/NewData[v1].xlsx")
str(data)
```
...
### Predictions Results 
```{r echo=FALSE, message=FALSE, warning=FALSE}
predictions = predict(fit.OLS, dataset[,-1])
postResample(pred = predictions, obs = dataset$retail)
```
...
predictions = predict(fit.rf, dataset[,-1])
postResample(pred = predictions, obs = dataset$retail)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Estimate Skill of a model using the Validation Set
predictions = predict(fit.OLS.r, dataset[,-1])
postResample(pred = predictions, obs = dataset$retail)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
lm = coef(lm(retail~teenpregnancyrate+work+under25, data = dataset)) #Same as running using caret with no control parameters...
summary(lm)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Estimate Skill of a model using the Validation Set
predictions = predict(fit.rf.r, dataset[,-1])
postResample(pred = predictions, obs = dataset$retail)
```
